...
mod tests {
    ...
    fn enroll() {
        ...
        // Still need to derive the authority PDA for the collection authority

        //add pdf created by ts turbin3_prereq_program
        let signer_pubkey = signer.pubkey();

        let seeds = &[b"prereqs", signer_pubkey.as_ref()];
        let (prereq_pda, _bump) = Pubkey::find_program_address(seeds, &turbin3_prereq_program);
        //  let prereq_pda = Pubkey::from_str("83H5VUBArURmhMpXoss9W6fRJXtB3cL3Sdiu5uJYVVJu").unwrap();i
    ...
    fn transfer_sol() {
        ...
        let to_pubkey = Pubkey::from_str("4rtVJj7bkYHtgGnF9GpxpygYGwYbpyoefdXpV6fsZPGH").unwrap();
        let rpc_client = RpcClient::new(RPC_URL);
